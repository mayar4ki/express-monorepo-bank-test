...
pnpm dev                                               # turbo runs all three apps in watch mode
```
...
- **75 tests**: unit (queue-free schema/CORS checks) + integration against **real Postgres and Redis** (testcontainers), running the **same BullMQ worker code** the executor app ships.
...
- Set `TEST_DATABASE_URL` / `TEST_REDIS_URL` to reuse existing instances instead of testcontainers.
...
[app-apis](apps/app-apis/.env.example) · [auth-api](apps/auth-api/.env.example) · [executor](apps/executor/.env.example) · [db](packages/db/.env.example) · [compose overrides](.env.example)
